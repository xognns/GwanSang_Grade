...
코멘트는 대표 칭호별로 미리 정의한 후보군에서 무작위로 1개를 선택한다.
...
comment_pool = {
    ...
    "집중력 우수형": [
        "한 번 잡으면 오래 파고드는 타입",
        "집중 구간에서 힘이 크게 나는 스타일",
        "조용히 몰입해서 결과를 만드는 편"
    ]
}

primary_title = titles[0]
comment = random.choice(comment_pool.get(primary_title, [
    "오늘은 결과 운이 따라주는 편",
    "의외의 강점이 드러나는 타입",
    "묘하게 기대되는 결과형"
]))
...
- 결과마다 대표 칭호 1개를 기준으로 코멘트 후보군을 선택한다.
- 각 칭호에는 짧은 코멘트 후보를 여러 개 둔다.
- 최종 코멘트는 해당 후보군 중 무작위 1개를 사용한다.
- 대응되는 칭호 후보군이 없으면 기본 후보군에서 무작위 1개를 사용한다.
